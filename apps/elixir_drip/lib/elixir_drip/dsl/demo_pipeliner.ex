defmodule DemoPipeliner do
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      def start_link(), do: Supervisor.start_link(__MODULE__, [], name: unquote(name))
      ...
      def show_pipeline_steps do
        IO.puts("Trying to directly get the @pipeline_steps is a no-go:")
        IO.inspect(@pipeline_steps)

        IO.puts("Here are the pipeline steps:")

        get_pipeline_steps()
        |> IO.inspect()
      end

      def show_default_values do
        IO.puts(
          "Defaults: count=#{@default_count}, min_demand=#{@default_min_demand}, max_demand=#{
            @default_max_demand
          }"
        )
      end
  ...
  defmacro start(producer, opts \\ []) do
    quote bind_quoted: [producer: producer, opts: opts] do
      options_and_args =
        get_options_and_args(opts, @default_count, @default_min_demand, @default_max_demand)
  ...
  def pipeline_specs(steps) do
    ...
    |> Enum.reduce([], fn step, worker_specs ->
      step =
        case Enum.empty?(worker_specs) do
          true ->
            step

          _ ->
            {names_to_subscribe, _} = Enum.at(worker_specs, -1)
            step ++ [names_to_subscribe: names_to_subscribe]
        end
  ...
  def get_worker_specs(
      ...
      do:
        get_worker_specs_with_subscriptions(
          producer_consumer,
          args: args,
          options: options,
          names_to_subscribe: names_to_subscribe
  ...
  defp get_worker_specs_with_subscriptions(
         consumer,
         args: args,
         options: options,
         names_to_subscribe: names_to_subscribe
  ...
  def get_options_and_args(opts, default_count, default_min_demand, default_max_demand) do
    options_and_args =
      fill_options_and_args(opts, default_count, default_min_demand, default_max_demand)
